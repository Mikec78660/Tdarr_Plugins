...
var plugin = function (args) {
    ...
    args.jobLog("Total subtitle streams found: ".concat(subtitleStreams.length));
    args.jobLog("Keeping first ".concat(streamsToKeep, " subtitle streams"));
    var keptCount = 0;
    var removedCount = 0;
    ...
    subtitleStreams.forEach(function (stream, index) {
        if (index >= streamsToKeep) {
            if (!stream.removed) {
                stream.removed = true;
                removedCount += 1;
                args.jobLog("Removing subtitle stream index ".concat(stream.index, " (position ").concat(index + 1, ")"));
            }
        }
        else {
            keptCount += 1;
            args.jobLog("Keeping subtitle stream index ".concat(stream.index, " (position ").concat(index + 1, ")"));
            // Add copy codec to preserve original subtitle format
            if (!stream.outputArgs) {
                stream.outputArgs = [];
            }
            stream.outputArgs.push("-c:".concat(stream.index), 'copy');
            args.jobLog("Setting copy codec for subtitle stream index ".concat(stream.index));
        }
    });
    args.jobLog("Kept ".concat(keptCount, " subtitle stream(s), removed ").concat(removedCount, " subtitle stream(s)"));
    return {
